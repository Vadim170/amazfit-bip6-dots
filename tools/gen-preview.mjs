...
function graph(g, spec, vals) {
  ...
  vals.forEach((v,i)=>{ const h = Math.max(3, Math.round(g.maxH*(v-lo)/span)); bar(spec.barsX+i*(g.barW+g.gap), g.baseY-h, g.barW, h, i===vals.length-1 ? '#d71921' : '#f0f0f0'); });
}
let aod = false;
function render() {
  ...
  place(A.DEGREE.src, w.degX, w.degY);
  const m = L.METRICS;
  num(String(SAMPLE.steps), A.NUMXS_WHITE, m.steps.centerX, m.numY, A.NUMXS.w);
  place(A.ICON_SRC.heartSm, m.hr.iconX, m.hr.iconY);
